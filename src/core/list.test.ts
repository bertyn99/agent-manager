...
describe("Phase 1: Enhanced List Command", () => {
  ...
  describe("Filter: Combinations", () => {
    it("should handle agent + type combination", () => {
      ...
      const filtered = extensions.filter((e) => agents.includes(e.agent) && types.includes(e.type));
    ...
    it("should handle agent + status combination", () => {
      ...
      // Filter requires BOTH agent AND status to match (AND logic)
      const filtered = extensions.filter(
        (e) => agents.includes(e.agent) && statuses.includes(e.enabled ? "enabled" : "disabled"),
      );

      // Only test1 matches both claude-code AND enabled
      expect(filtered.length).toBe(1);
      expect(filtered[0].name).toBe("test1");
    });
    ...
    it("should handle type + status combination", () => {
      ...
      // Filter requires BOTH type AND status to match (AND logic)
      const filtered = extensions.filter(
        (e) => types.includes(e.type) && statuses.includes(e.enabled ? "enabled" : "disabled"),
      );

      // Only test1 matches both mcp AND enabled
      expect(filtered.length).toBe(1);
      expect(filtered[0].name).toBe("test1");
    });
    ...
    it("should handle agent + type + status combination", () => {
      ...
      const filtered = extensions.filter(
        (e) =>
          agents.includes(e.agent) ||
          types.includes(e.type) ||
          statuses.includes(e.enabled ? "enabled" : "disabled"),
      );
