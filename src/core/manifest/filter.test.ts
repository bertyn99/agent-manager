...
import { addMcpToManifest, removeMcpFromManifest } from "./mcp.js";
import { addSkillOriginGroup, updateSkillInOrigin } from "./skills.js";
...
describe("Manifest Helper Functions", () => {
  ...
  it("should add skill origin group - validation test", () => {
    expect(() =>
      addSkillOriginGroup("/tmp", "https://github.com/user/repo", "/skills", "main", {
        include: [],
        exclude: [],
      }),
    ).not.toThrow();
  ...
  it("should update skill in origin - validation test", () => {
    expect(() =>
      updateSkillInOrigin("/tmp", "https://github.com/user/repo", "skill-name", ["claude-code"]),
    ).toThrow();
  });
});
