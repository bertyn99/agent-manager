...
describe("Extended List Filters", () => {
  ...
  describe("Filter behavior - search", () => {
    ...
    it("should be case-insensitive", () => {
      // Search in name - no extensions have 'GITHUB' in name, so expect 0
  ...
  describe("Filter behavior - sort", () => {
    it("should sort by name alphabetically", () => {
      ...
      expect(sorted[2].name).toBe("test-skill");
      expect(sorted[3].name).toBe("zai-mcp");
    });
    ...
    it("should sort by type", () => {
      ...
      // First should be command (alphabetically first)
      expect(sorted[0].type).toBe("command");
      // skill should be last (alphabetically last)
      expect(sorted[sorted.length - 1].type).toBe("skill");
    });
  ...
  describe("Filter behavior - limit", () => {
    it("should return limited number of results", () => {
      ...
      expect(limited[1].name).toBe("test-skill");
    });
